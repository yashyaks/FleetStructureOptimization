...
class MultiObjectiveFleetOptimizer:
    ...
    def _process_size_distance(self, size_distance: Tuple) -> List[Dict]:
        results = []
    ...
    def get_optimized_results(self, year) -> pd.DataFrame:
        ...
        with mp.Pool(processes=mp.cpu_count()) as pool:
            all_results = pool.map(self._process_size_distance, self.vehicles_by_size_distance.keys())
    ...
    # Alternative implementation using ProcessPoolExecutor (newer API)
    def get_optimized_results_concurrent(self, year) -> pd.DataFrame:
        import concurrent.futures
        
        all_results = []
        
        with concurrent.futures.ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
            future_to_size_distance = {executor.submit(self._process_size_distance, size_distance): size_distance 
                                      for size_distance in self.vehicles_by_size_distance.keys()}
            
            for future in concurrent.futures.as_completed(future_to_size_distance):
                size_distance = future_to_size_distance[future]
                try:
                    results = future.result()
                    all_results.extend(results)
                except Exception as exc:
                    print(f"Size-distance {size_distance} generated an exception: {exc}")
        
        df = pd.DataFrame(all_results)
        return df
    
    # def get_optimized_results_with_timeline(self, year) -> pd.DataFrame:
    
    #     start_time = time.time()
    #     timeline = defaultdict(list)
        
    #     def process_with_timeline(size_distance):
    #         pid = os.getpid()
    #         task_start = time.time() - start_time
            
    #         result = self._process_size_distance(size_distance)
            
    #         task_end = time.time() - start_time
    #         timeline[pid].append((size_distance, task_start, task_end))
    #         return result
        
    #     with concurrent.futures.ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
    #         future_to_size_distance = {
    #             executor.submit(process_with_timeline, sd): sd 
    #             for sd in self.vehicles_by_size_distance.keys()
    #         }
    ...
    #     for pid, tasks in timeline.items():
    #         print(f"Process {pid}:")
    #         for task, start, end in tasks:
    #             print(f"  {task}: {start:.2f} -> {end:.2f} ({end-start:.2f}s)")
...
#     optimizer = MultiObjectiveFleetOptimizer(data, emission_weight=0.6, cost_weight=0.4)
    
#     # Choose which parallelization method to use
#     results_df = optimizer.get_optimized_results(2023)
#     # Or alternatively:
#     # results_df = optimizer.get_optimized_results_concurrent(2023)
    
#     # Save results
#     results_df.to_csv(f"optimized_fleet_{2023}.csv", index=False)
