...
from tradeoff_topsis_parellel import MultiObjectiveFleetOptimizer
...
def main():
    ...
    for year in range(2023, 2039):
        ...
        if os.path.exists(f"data/output/tradeoff/topsis/multi_objective_fleet_allocation_parallel_{(year-1)}.csv"):
            print("Merging with previous year vehicles")
            df1 = pd.read_csv(f"data/output/tradeoff/topsis/multi_objective_fleet_allocation_parallel_{(year-1)}.csv")
        ...
        df.to_csv(f'data/output/tradeoff/topsis/multi_objective_fleet_allocation_parallel_{year}.csv', index=False)
        ...
        df.to_csv(f'data/output/tradeoff/topsis/multi_objective_fleet_allocation_eval__parallel_{year}.csv', index=False)
        ...
        summary_df.to_csv('data/output/tradeoff/topsis/multiobjective_summary_parallel.csv')
        print("Summary generated")
        
        engine = sqlops.create_sqlalchemy_engine(connection_string)
        df.to_sql(f'multi_objective_fleet_allocation_eval__parallel_{year}', con=engine, if_exists='replace') 
        summary_df.to_sql('multiobjective_summary_parallel.csv', con=engine, if_exists='replace')
        print()
    ...
    result.to_sql(f'combined_multi_objective_fleet_allocation_eval_parallel', con=engine, if_exists='replace') 
